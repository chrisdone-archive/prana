...
import qualified Paths_prana_compiler
...
main = do
   ...
   GHC.defaultErrorHandler defaultFatalMessager defaultFlushOut $ do
    ...
    if elem "--version" argv00
       then do putStrLn ("Prana " ++ Data.Version.showVersion Paths_prana_compiler.version)
               exitSuccess
       else return ()
    case lookup "STACK_EXE" env of
      Just{} -> return ()
      Nothing ->
        hPutStr stderr ("WARNING: it is HIGHLY RECOMMENDED to use prana with stack:\n\n"
                       ++ "  To install:\n"
                       ++ "    stack build prana\n\n"
                       ++ "  To run with no project:\n"
                       ++ "    stack exec prana\n"
                       ++ "  To run with your project:\n"
                       ++ "    stack ghci --with-ghc prana-compiler\n\n")
